...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
          ...
          IconButton(
            ...
            onPressed: () async {
              try {
                ...
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Грешка при вчитување: \$e')));
              }
            },
          )
        ],
      ...
      child: TextField(
        decoration: const InputDecoration(prefixIcon: Icon(Icons.search), hintText: 'Пребарај категории'),
    ...
    final cats = (snapshot.data ?? []).where((c) => c.name.toLowerCase().contains(_query.toLowerCase())).toList();
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, i) => GestureDetector(
        onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => CategoryScreen(category: cats[i].name))),
